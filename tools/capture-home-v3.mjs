...
try {
  for (const capture of captures) {
    ...
    if (capture.viewport.width >= 1024) {
      ...
      assert.ok(capture.viewport.width - result.shell.right >= 15 && capture.viewport.width - result.shell.right <= 17, `${capture.file}: shell right ${capture.viewport.width - result.shell.right}`);
